...
func NewCommand() *cobra.Command {
	...
	cmd.Flags().BoolVar(&archivePurge, "archive-purge", false, "Purge binary logs once archived (the active purge gate). Disabled by default: mysqld's own binlog_expire_logs_seconds governs expiry so binlogs stay available for lagged or returning replicas")
