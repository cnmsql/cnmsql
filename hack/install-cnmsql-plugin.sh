...
Example:
  curl -sSfL https://github.com/cnmsql/cnmsql/raw/main/hack/install-cnmsql-plugin.sh | sh -s -- -b /usr/local/bin
...
get_latest_tag() {
  ...
  tag=$(curl -sSfL "https://api.github.com/repos/cnmsql/cnmsql/releases/latest" \
...
install_binary() {
  ...
  log_info "installing kubectl-cnmsql ${TAG} for ${OS}/${ARCH}"

  NAME="kubectl-cnmsql_${VERSION}_${OS}_${ARCH}"
  if [ "$OS" = "windows" ]; then
    ARCHIVE="${NAME}.zip"
  else
    ARCHIVE="${NAME}.tar.gz"
  ...
  BASE_URL="https://github.com/cnmsql/cnmsql/releases/download/${TAG}"
  ...
  mkdir -p "${BINDIR}"
  install -m 0755 "${TMPDIR}/kubectl-cnmsql" "${BINDIR}/kubectl-cnmsql"
  log_info "installed ${BINDIR}/kubectl-cnmsql"

  cat >"${BINDIR}/kubectl_complete-cnmsql" <<'COMPLETION'
#!/usr/bin/env sh
exec kubectl-cnmsql __complete "$@"
COMPLETION
  chmod +x "${BINDIR}/kubectl_complete-cnmsql"
  log_info "installed ${BINDIR}/kubectl_complete-cnmsql (tab completion)"
}
