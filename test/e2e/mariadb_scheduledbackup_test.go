...
// The MariaDB counterpart of the "Scheduled backups" suite. It proves the
// ScheduledBackup controller drives mariabackup runs on a MariaDB cluster: an
...
var _ = Describe("MariaDB scheduled backups", Ordered, Label("feature", "mariadb"), func() {
	const (
		...
		immediateLabel      = "mysql.cnmsql.co/immediate-backup"
	)
	...
	It("fires an immediate backup and keeps the cron cadence", func() {
		...
		immediate, err := kubectl("get", "backups", "-n", testNamespace,
			...
			"-o", "jsonpath={.items[0].metadata.labels."+jsonpathEscape(immediateLabel)+"}")
