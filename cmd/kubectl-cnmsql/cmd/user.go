...
func newUserCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Create, alter, drop and list MySQL users via the instance manager control API on the primary.\n\n" +
			...
			"(cnmsql_*) cannot be managed.",
		Example: `  # Create a user (prompts for password)
  kubectl cnmsql user create cluster-sample --name=app

  # Create a user with a password from a secret on stdin
  kubectl get secret app-creds -o jsonpath='{.data.password}' | base64 -d | \
    kubectl cnmsql user create cluster-sample --name=app --password-stdin

  # Create a superuser with X509 TLS requirement
  kubectl cnmsql user create cluster-sample --name=admin --superuser --require-tls=x509
...
PRIVILEGES, or --privileges for specific grants.`,
  ...
  kubectl cnmsql user create cluster-sample --name=reader --privileges=SELECT --on=mydb.*
...
is read from stdin (--password-stdin) or prompted on the terminal.`,
  ...
  kubectl cnmsql user alter cluster-sample --name=app --password-stdin < newpass.txt

  # Set TLS requirement to X509
  kubectl cnmsql user alter cluster-sample --name=app --require-tls=x509`,
		Args:              cobra.MaximumNArgs(1),
		ValidArgsFunction: completeClusterArg,
		RunE: func(cmd *cobra.Command, args []string) error {
			name, _ := cmd.Flags().GetString("name")
