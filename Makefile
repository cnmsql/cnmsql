...
.PHONY: vulncheck
vulncheck: govulncheck ## Scan for known vulnerabilities with govulncheck
	"$(GOVULNCHECK)" ./...
...
$(GOLANGCI_LINT): $(LOCALBIN)
	...
	@test -f .custom-gcl.yml && { \
		...
		GOTOOLCHAIN=go$$(go list -m -f '{{.GoVersion}}') $(GOLANGCI_LINT) custom --destination $(LOCALBIN) --name golangci-lint-custom && \
