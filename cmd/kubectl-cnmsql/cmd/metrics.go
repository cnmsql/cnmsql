...
func newMetricsCommand() *cobra.Command {
  ...
  kubectl cnmsql metrics cluster-sample

  # Scrape metrics from a specific instance
  kubectl cnmsql metrics cluster-sample cluster-sample-2

  # Watch metrics refresh every 2 seconds, filtered to thread counters
  kubectl cnmsql metrics -w --filter=mysql_global_status_threads cluster-sample`,
